...
const Deposit = ({navigation, route}) => {
  ...
  const ifsc = route.params.ifsc ;
    ...
    const deposit = (accNo,uName,email,password,ifsc) => {
      ...
      if(isNaN(amount)){
        ...
        axios({
            method: 'POST',
            url: 'http://192.168.0.105:8085/Customer/deposit/' + accNo +"/"+ amount, 
        }).then(function(response){
            alert(JSON.stringify(response.data));
            setAmount("");
            navigation.navigate("Home",{accNo,uName,email,password,ifsc});
...
return (
 ...
 <Text style={styles.headerTxt}>WELCOME {uName}</Text>
      <View style={styles.subView}>
        <Text style={styles.subTxt}>Deposit</Text>
        <TextInput style={styles.nameInput} placeholder="Enter Amount to Deposit" value={amount} onChangeText={(e) => setAmount(e)} keyboardType='numeric' />
 
        <TouchableOpacity style={styles.btn} onPress={() => deposit(accNo,uName,email,password,ifsc)}>
